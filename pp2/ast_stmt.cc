...
CaseStmt::CaseStmt(Expr *c, List<Stmt *> *b)
{
    Assert(c!=NULL);
...
SwitchStmt::SwitchStmt(Expr *h, List<CaseStmt *> *cl, DefaultStmt *d)
{
    Assert(h != NULL && cl != NULL);
    (head = h)->SetParent(this);
    (caseList = cl)->SetParentAll(this);
    if (d)
